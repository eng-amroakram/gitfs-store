<div class="container-fluid p-5 rounded-3">
    ...
    <div class="mb-4 bg-light">
        ...
        <nav class="d-flex">
            <h6 class="mb-0">
                ...
                <span>/</span>
                <a href="{{ route('admin.panel.suppliers.list', ['lang' => app()->getLocale()]) }}"
                    class="text-reset"><u>{{ __('Suppliers') }}</u></a>
            </h6>
        </nav>
        <!-- Breadcrumb -->
    </div>
        ...
        <div class="col-md-9 d-flex gap-3" wire:ignore>
            <div class="col-md-3 mb-2">
                ...
                <div class="form-outline" data-mdb-input-init>
                    ...
                    <label class="form-label" for="search">{{ __('Search by name, phone, email') }}</label>
        ...
        <div class="col-md-3 d-flex justify-content-end gap-2">
            ...
            @if (count($selectedSuppliers) > 0)
                <button class="btn btn-danger btn-sm" wire:click="confirmDeleteSelected" wire:loading.attr="disabled"
                    ...
                    <i class="fas fa-trash-alt"></i> {{ __('Delete') }} ({{ count($selectedSuppliers) }})
                </button>
            @endif
        </div>
    </div>
    ...
    <!-- Start Suppliers Table Responsive Bordered -->
                    ...
                    <tr>
                        <td class="text-center">
                            <div class="form-check font-size-16 d-flex justify-content-center align-items-center">
                                <input type="checkbox" class="form-check-input" value="{{ $supplier->id }}"
                        ...
                        <td>
                            @if ($supplier->synced_at)
                                <span
                                    class="badge badge-success">{{ $supplier->synced_at->format('Y-m-d H:i') }}</span>
                            @else
                        ...
                        <td>
                            ...
                            <span wire:loading.remove wire:target="edit({{ $supplier->id }})">
                                <a href="#edit" wire:click="edit({{ $supplier->id }})"
                            ...
                            <!-- Show Icon -->
                            <span wire:loading.remove wire:target="show({{ $supplier->id }})">
                                <a href="#" wire:click="show({{ $supplier->id }})"
                                    class="text-primary fa-lg me-2 ms-2" title="{{ __('Show') }}">
                                    <x-icons.show />
                                </a>
                            </span>
                            <span wire:loading wire:target="show({{ $supplier->id }})">
    ...
    <!-- End Suppliers Table Responsive Bordered -->

    <!-- Table Pagination -->
    <div class="d-flex justify-content-between mt-4">
        <nav aria-label="...">
            <ul class="pagination pagination-circle">
                {{ $suppliers->withQueryString()->onEachSide(0)->links() }}
            </ul>
        </nav>
