            ...
            <form wire:submit.prevent="update">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" id="payment_reference" wire:model.defer="payment_reference"
...
@push('scripts')
    <script>
        $(document).ready(function() {
            var $paymentableType = "{{ $paymentable_type }}" + '';
            var $paymentableId = "{{ $paymentable_id }}" + '';
            var $method = "{{ $method }}" + '';
            var $allSales = @json($allSales);
            var $allReservations = @json($allReservations);

            console.log($allSales);
            console.log($allReservations);
            console.log($paymentableId);

            if ($paymentableType) {
                const $paymentableTypeSelector = document.querySelector("#paymentable_type");
                const $paymentableTypeSelectInstance = mdb.Select.getInstance($paymentableTypeSelector);
                $paymentableTypeSelectInstance.setValue($paymentableType);
            }

            if ($method) {
                const $methodSelector = document.querySelector("#method");
                const $methodSelectInstance = mdb.Select.getInstance($methodSelector);
                $methodSelectInstance.setValue($method);
            }

            if ($paymentableId) {
                const $paymentableIdSelector = document.querySelector("#paymentable_id");
                const $paymentableIdSelectInstance = mdb.Select.getInstance($paymentableIdSelector);
                $paymentableIdSelectInstance.setValue($paymentableId);
            }
